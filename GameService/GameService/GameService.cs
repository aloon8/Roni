    ...
    public class GameServiceClass : IGameService
    {
        ...
        public void StartGameBetweenPlayers(string p1, string p2)
        {
            ...
            using (var ctx = new fourinrowDB_RoniShoseov_EilonOsherContext())
            {
                ...
                GameZone gm = new GameZone(p1, p2, avilableClinets[p1], avilableClinets[p2]);
            }
        ...
        public void updateStartGameDB(string user1, string user2)
        {
            using (var ctx = new fourinrowDB_RoniShoseov_EilonOsherContext())
            {
                var findUser1 = (from u in ctx.SingleGames
                                where u.Player1_Name == user1
                                || u.Player2_Name == user1
                                select u).FirstOrDefault();
                var findUser2 = (from u in ctx.SingleGames
                                 where u.Player2_Name == user2
                                 || u.Player1_Name == user2
                                 select u).FirstOrDefault();
